...
struct vnc_server {
    ...
    // Internal framebuffer copy (always in 32-bit BGRA format, i.e., B in low byte, G, R, X in high byte)
    ...
    // Cached pollfd array (reused across vnc_server_poll calls to avoid malloc/free)
...
static inline int pixel_format_equal(const vnc_pixel_fmt_t* a, const vnc_pixel_fmt_t* b) {
    return a->bits_per_pixel == b->bits_per_pixel &&
           a->depth == b->depth &&
           a->big_endian_flag == b->big_endian_flag &&
           a->true_color_flag == b->true_color_flag &&
           a->red_max == b->red_max &&
...
// Convert a single pixel from default internal format (BGRA8888) to client format
static inline void convert_pixel(uint32_t src_pixel, uint8_t* dst, const vnc_pixel_fmt_t* fmt) {
    ...
    uint32_t val = (cr << fmt->red_shift) | (cg << fmt->green_shift) | (cb << fmt->blue_shift);
