...
// Mirrors src/leanrfb_h264.c's structure and signatures, targeting VP9 instead.
// See docs/custom/rfb_vp9_extension.md.
//
// Note: NVIDIA NVENC has no VP9 encoder, so unlike the H.264/H.265 paths there is no
// NVENC attempt here — the fallback chain is VA-API (Intel/AMD) GPU encoding, then
// software libvpx-vp9.
...
typedef struct {
    ...
    int height;
    int pts;
    int is_hw; // 1 = VA-API, 0 = Software libvpx-vp9
    int force_keyframe; // set by vnc_vp9_encoder_force_keyframe(), consumed on next encode
} vnc_vp9_encoder_t;

// Color space conversion helper functions (fast integer logic) — duplicated from
// leanrfb_h264.c to keep each codec's encoder self-contained.
...
void* vnc_vp9_encoder_create(int width, int height, int fps, int quality) {
    (void)quality;
    vnc_vp9_encoder_t* enc = calloc(1, sizeof(vnc_vp9_encoder_t));
    if (!enc) return NULL;
    ...
    // 1. Try to initialize Intel/AMD VA-API GPU Encoder (no NVENC VP9 encoder exists)
    const AVCodec* codec = avcodec_find_encoder_by_name("vp9_vaapi");
    if (codec) {
    ...
    // 2. Fallback: CPU Software libvpx-vp9 via libavcodec
    ...
    if (codec) {
        ...
        av_opt_set(enc->codec_ctx->priv_data, "deadline", "realtime", 0); // lowest-latency rate control mode
        av_opt_set_int(enc->codec_ctx->priv_data, "cpu-used", 8, 0);      // fastest speed/quality tradeoff
...
int vnc_vp9_encoder_encode(void* enc_ptr, const uint32_t* fb, uint8_t** out_data, int* out_len, int* is_keyframe, int* pts_out) {
    vnc_vp9_encoder_t* enc = (vnc_vp9_encoder_t*)enc_ptr;
    if (!enc || !fb) return -1;
    ...
    if (enc->is_hw == 1) { // VA-API hardware context texture upload
        ...
        if (err < 0) {
            ...
            fprintf(stderr, "[VNC SERVER] VP9: av_hwframe_get_buffer failed: %s\n", errbuf);
            return -1;
        }
        ...
        if (err < 0) {
            char errbuf[128];
            av_strerror(err, errbuf, sizeof(errbuf));
            fprintf(stderr, "[VNC SERVER] VP9: av_hwframe_transfer_data failed: %s\n", errbuf);
            return -1;
        }
    ...
    if (ret < 0) {
        ...
        fprintf(stderr, "[VNC SERVER] VP9: avcodec_send_frame failed: %s\n", errbuf);
        return -1;
    }
    ...
    if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
        ...
        *is_keyframe = 0;
        ...
        char errbuf[128];
        av_strerror(ret, errbuf, sizeof(errbuf));
        fprintf(stderr, "[VNC SERVER] VP9: avcodec_receive_packet failed: %s\n", errbuf);
        return -1;
    }
